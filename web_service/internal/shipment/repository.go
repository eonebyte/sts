...
func (r *oraRepo) GetDailyProgress(ctx context.Context, from, to time.Time) ([]ShipmentProgress, error) {
    ...
    SELECT 
	    ...
	    MAX(CASE WHEN ase.EVENTTYPE = 'HO: DRIVER_CHECKIN' OR io.ADW_TMS_ID IS NOT NULL OR io.SPPNO IS NOT NULL THEN 1 ELSE 0 END) AS ONDRIVER,
	    MAX(CASE WHEN ase.EVENTTYPE = 'HO: DRIVER_CHECKOUT' OR io.ADW_TMS_ID IS NOT NULL OR io.SPPNO IS NOT NULL THEN 1 ELSE 0 END) AS ONCUSTOMER,
        MAX(CASE WHEN ase.EVENTTYPE = 'RE: DPK_FROM_DRIVER' OR io.ADW_TMS_ID IS NOT NULL OR io.SPPNO IS NOT NULL THEN 1 ELSE 0 END) AS OUTCUSTOMER,
        MAX(CASE WHEN ase.EVENTTYPE = 'HO: DPK_TO_DEL' OR io.ADW_TMS_ID IS NOT NULL OR io.SPPNO IS NOT NULL THEN 1 ELSE 0 END) AS COMEBACKDPK,
        MAX(CASE WHEN ase.EVENTTYPE = 'HO: DEL_TO_MKT' OR io.ADW_TMS_ID IS NOT NULL OR io.SPPNO IS NOT NULL THEN 1 ELSE 0 END) AS COMEBACKDEL,
        MAX(CASE WHEN ase.EVENTTYPE = 'HO: MKT_TO_FAT' OR io.ADW_TMS_ID IS NOT NULL AND io.SPPNO IS NOT NULL THEN 1 ELSE 0 END) AS COMEBACKMKT,
        MAX(CASE WHEN ase.EVENTTYPE = 'RE: FAT_FROM_MKT' OR io.ADW_TMS_ID IS NOT NULL AND io.SPPNO IS NOT NULL AND io.C_INVOICE_ID IS NOT NULL THEN 1 ELSE 0 END) AS COMEBACKFAT
    FROM M_INOUT io 
